package examples.libadwaita.demo_official.styles;
...
public class AdwDemoPageStyles {
    private final static Str TYPE_NAME = new Str(AdwDemoPageStyles.class.getSimpleName());
    ...
    public synchronized static long getTypeID() {
        if (type == 0) {
            type = TypeSystem.registerClass(Bin.getTypeID(), TYPE_NAME, 0, (__self, g_class, class_data) -> {
                ...
                widgetClass.setTemplateOrExit("/adw_demo/adw-demo-page-styles.ui");

                widgetClass.installAction("demo.run", null, (__self1, widget, action_name, parameter) -> {
                    var window = new AdwStyleDemoWindow();
