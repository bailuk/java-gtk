...
public class ClassModel extends Model {
    ...
    private List<SignalModel> signals = new ArrayList();
    ...
    public ClassModel(StructureTag structure, NameSpaceModel nameSpace) {
        ...
        for (SignalTag signal: structure.getSignals()) {
            addIfSupported(signals, new SignalModel(signal));
        }
    }
    ...
    public boolean hasNativeCalls() {
        return isNameSpaceSupported() && isClassType() && (methods.size() >0 || privateFactories.size() > 0);
    }
    ...
    public void write(CodeWriter writer) throws IOException {
        ...
        if (isClassType()) {
            writer.next();
            writer.writeInternalConstructor(getName());
        }
        ...
        if (isClassType()) {
            ...
            for (SignalModel s : signals) {
                writer.writeSignal(s);
            }
        } else {
            for (MethodModel m : methods) {
    ...
    public String getJniMethodName(MethodModel m) {
            return Configuration.JNI_METHOD_NAME_BASE + nameSpace.getNamespace()  +"_" + getImpName() + "_" + m.getName();
    }
}
